...
class TestTeamMembership:
    ...
    def test_list_by_user_single_team(self, tmp_path):
        repo = make_repo(tmp_path)
...
class TestResolveTeamSkills:
    def _make_skill(self, skill_id, name="skill"):
        s = MagicMock(); s.id = skill_id; s.name = name; return s

    def test_no_team_membership_returns_none(self, tmp_path):
        repo = make_repo(tmp_path)
        skill_repo = MagicMock()
        assert resolve_team_skills("orphan", repo, skill_repo, None) is None
    ...
    def test_single_team_filters_skills(self, tmp_path):
        ...
        local, remote = resolve_team_skills("alice", repo, skill_repo, None)
    ...
    def test_multi_team_union_of_skills(self, tmp_path):
        ...
        local, _ = resolve_team_skills("alice", repo, skill_repo, None)
        assert {s.id for s in local} == {"skill-a", "skill-b"}

    def test_team_with_remote_repo_syncs(self, tmp_path):
        repo = make_repo(tmp_path)
        team = repo.create(name="T", skill_repo_url="https://example.com/skills")
        repo.add_member(team.team_id, "alice")
        skill_repo = MagicMock(); skill_repo.list.return_value = []
        remote_skill = self._make_skill("remote-1")
        mock_repo = MagicMock(); mock_repo.sync.return_value = [remote_skill]
        with patch("app.teams.RemoteSkillRepository", return_value=mock_repo):
            _, remote = resolve_team_skills("alice", repo, skill_repo, None)
        assert remote == [remote_skill]

    def test_failed_remote_sync_is_skipped(self, tmp_path):
        repo = make_repo(tmp_path)
        team = repo.create(name="T", skill_repo_url="https://bad.example.com")
        repo.add_member(team.team_id, "alice")
        skill_repo = MagicMock(); skill_repo.list.return_value = []
        mock_repo = MagicMock(); mock_repo.sync.side_effect = RuntimeError("failed")
        with patch("app.teams.RemoteSkillRepository", return_value=mock_repo):
            _, remote = resolve_team_skills("alice", repo, skill_repo, None)
        assert remote == []
    ...
    def test_deduplicates_remote_repo_url(self, tmp_path):
        ...
        repo.add_member(t2.team_id, "alice")
        skill_repo = MagicMock(); skill_repo.list.return_value = []
        mock_repo = MagicMock(); mock_repo.sync.return_value = []
        with patch("app.teams.RemoteSkillRepository", return_value=mock_repo) as MockCls:
            resolve_team_skills("alice", repo, skill_repo, None)
